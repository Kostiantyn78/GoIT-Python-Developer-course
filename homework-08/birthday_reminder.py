...
def get_birthdays_per_week(users):
    ...
    for shift_day in range(0, 7):  # through 7 days, starting with the current day
        for user in users:
            if user['birthday'] == start_day + datetime.timedelta(shift_day):
                    lucky_names.append(user['name'])

        # if the current day is Saturday or Sunday, accumulate the names and move to the next day
        if (start_day + datetime.timedelta(shift_day)).weekday() > 4:
            continue

        if lucky_names:
            print(f'{(start_day + datetime.timedelta(shift_day)).strftime("%A")}: ', end='')
            print(*lucky_names, sep=', ')
            lucky_names = []

    # output names for Saturday and Sunday, if the search started on Monday
    if lucky_names:
        print(f'{(start_day + datetime.timedelta(7)).strftime("%A")}: ', end='')  # just word Monday on all languages
        print(*lucky_names, sep=', ')
