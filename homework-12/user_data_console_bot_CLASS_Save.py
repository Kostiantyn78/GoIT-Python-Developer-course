...
@input_error
def save_to_file(contact_data, book: AddressBook):

    file_name = 'address_book.bin'
    with open(file_name, 'wb') as fh:
        dump(book.data, fh)

    return f'Contacts were successfully saved to file {file_name}'

@input_error
def search_contact(contact_data, book: AddressBook):

    rezult = ''
    str_for_search = contact_data[0].lower()

    for name, record in book.data.items():

        if str_for_search in name and record.name.value not in rezult:

            rezult += f"Contact name: {record.name}, " \
                      f"phones: {'; '.join(p.value for p in record.phones)}, " \
                      f"birthday: {record.birthday}\n"
        else:
            for phone in record.phones:
                if str_for_search in phone.value and record.name.value not in rezult:
...
def main():
    ...
    - save
    - search <name> or <phone> or <part 'name' or 'phone'>
            ''')
