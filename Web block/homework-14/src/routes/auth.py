...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    ...
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=messages.WRONG_EMAIL)
...
async def forget_password(
    ...
    It then checks if that email exists in our database, and if it does not exist, we return a 500 error code with
    an appropriate message. If it does exist however, we call our background task function send_email_reset_password
    and pass in the users' username and base url as parameters.
    ...
    """
    try:
        user = await repository_users.get_user_by_email(fpr.email, db)
        if user is None:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                                detail=messages.WRONG_EMAIL)
...
async def reset_password(token: str, new_password: str, db: AsyncSession = Depends(get_db)):
    ...
    await repository_users.new_password(user.email, password, db)
...
async def get_user_by_reset_token(token: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if user is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=messages.VERIFICATION_ERROR)
        if not user.confirmed:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=messages.EMAIL_NOT_CONFIRMED)
