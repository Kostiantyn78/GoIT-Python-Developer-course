...
from src.repository.users import (get_user_by_email, create_user, update_token, update_avatar_url, confirmed_email,
                                  new_password)
...
class TestRepositoryUsers(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.body = UserSchema(username="TestUser", email="TestUser_mail@gmail.com", password="555555")
    ...
    async def test_get_user_by_email(self):
        user = User(email="test_mail@gmail.com", password="test_passw")
        mocked_user = Mock()
        mocked_user.scalar_one_or_none.return_value = user
        self.session.execute.return_value = mocked_user
        result = await get_user_by_email(email="test_mail@gmail.com", db=self.session)
    ...
    async def test_update_avatar_url(self):
        ...
        mocked_user.scalar_one_or_none.return_value = user
        self.session.execute.return_value = mocked_user
        result = await update_avatar_url(user.email, "new_url_for_avatar", self.session)
        self.session.commit.assert_called_once()
